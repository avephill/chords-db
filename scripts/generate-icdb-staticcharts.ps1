# Parse ICDb _raw_v1..v4.html (StaticCharts alt=...) from scripts/icdb-data/<instrument>/<Key>/.
# Emits chord *.js + key index.js into src/db/<instrument>/chords/<Key>/.
# Optionally writes readable debug text into raw workspace.
...
function AltToFrets([string]$alt) {
    ...
    if ($parts.Count -ne $StringCount) { return $null }
    $out = ""
    foreach ($p in $parts) {
...
foreach ($k in $Keys) {
    $outDirPath = Join-Path $ChordsRoot $k.dir
...
"@
        Set-Content -Path (Join-Path $outDirPath $fn) -Value $body -Encoding UTF8 -NoNewline
    }
    ...
    Set-Content -Path (Join-Path $outDirPath "index.js") -Value $idx -Encoding UTF8 -NoNewline
