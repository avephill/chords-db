...
import banjoDoubleC from './banjo-double-c';
import { strChord2array, processString } from '../tools';

describe('Banjo (double C) Chords', () => {
  describe('Strings', () => {
    it('Should have 5 strings', () =>
      expect(banjoDoubleC.main.strings).toEqual(5));
    it('Should have 5 frets on chord', () =>
      expect(banjoDoubleC.main.fretsOnChord).toEqual(5));
  });

  describe('Types', () => {
    banjoDoubleC.suffixes.map((suffix) =>
  ...
  describe('G major', () => {
    it('Should have positions with 5-string frets', () => {
      const Gmajor = banjoDoubleC.chords.G.find(
  ...
  Object.keys(banjoDoubleC.chords).map((key) =>
    describe(`Key ${key}`, () => {
      const chords = banjoDoubleC.chords[key];
            ...
            chord.positions.map((position, index) => {
              ...
              describe('Frets', () => {
                ...
                it(`The ${index + 1} position frets array should have values lower than 16`, () =>
                  expect(Math.max(...frets)).toBeLessThan(16));
                it(`The ${index + 1} position frets array should have at most ${banjoDoubleC.main.fretsOnChord} frets of distance`, () =>
                  expect(
                    effectiveFrets.length === 0
                      ? 0
                      : Math.max(...effectiveFrets) - Math.min(...effectiveFrets)
                  ).toBeLessThanOrEqual(banjoDoubleC.main.fretsOnChord));
              });
