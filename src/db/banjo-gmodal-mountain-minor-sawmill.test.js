...
import banjoGModalMountainMinorSawmill from './banjo-gmodal-mountain-minor-sawmill';
import { strChord2array, processString } from '../tools';

describe('Banjo (G modal / mountain minor / sawmill) Chords', () => {
  describe('Strings', () => {
    it('Should have 5 strings', () =>
      expect(banjoGModalMountainMinorSawmill.main.strings).toEqual(5));
    it('Should have 5 frets on chord', () =>
      expect(banjoGModalMountainMinorSawmill.main.fretsOnChord).toEqual(5));
  });

  describe('Types', () => {
    banjoGModalMountainMinorSawmill.suffixes.map((suffix) =>
  ...
  describe('G major', () => {
    it('Should have positions with 5-string frets', () => {
      const Gmajor = banjoGModalMountainMinorSawmill.chords.G.find(
        (chord) => chord.suffix === 'major'
      );
      expect(Gmajor.positions.length).toBeGreaterThan(0);
      Gmajor.positions.forEach((position) => {
  ...
  Object.keys(banjoGModalMountainMinorSawmill.chords).map((key) =>
    describe(`Key ${key}`, () => {
      const chords = banjoGModalMountainMinorSawmill.chords[key];
            ...
            chord.positions.map((position, index) => {
              ...
              describe('Frets', () => {
                ...
                it(`The ${index + 1} position frets array should have values lower than 16`, () =>
                  expect(Math.max(...frets)).toBeLessThan(16));
                it(`The ${index + 1} position frets array should have at most ${banjoGModalMountainMinorSawmill.main.fretsOnChord} frets of distance`, () =>
                  expect(
                    effectiveFrets.length === 0
                      ? 0
                      : Math.max(...effectiveFrets) - Math.min(...effectiveFrets)
                  ).toBeLessThanOrEqual(banjoGModalMountainMinorSawmill.main.fretsOnChord));
              });
