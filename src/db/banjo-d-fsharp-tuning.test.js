...
import banjoDFsharpTuning from './banjo-d-f#-tuning';
import { strChord2array, processString } from '../tools';

describe('Banjo (D / F# tuning) Chords', () => {
  describe('Strings', () => {
    it('Should have 5 strings', () =>
      expect(banjoDFsharpTuning.main.strings).toEqual(5));
    it('Should have 5 frets on chord', () =>
      expect(banjoDFsharpTuning.main.fretsOnChord).toEqual(5));
  });

  describe('Types', () => {
    banjoDFsharpTuning.suffixes.map((suffix) =>
  ...
  describe('G major', () => {
    it('Should have positions with 5-string frets', () => {
      const Gmajor = banjoDFsharpTuning.chords.G.find(
  ...
  Object.keys(banjoDFsharpTuning.chords).map((key) =>
    describe(`Key ${key}`, () => {
      const chords = banjoDFsharpTuning.chords[key];
            ...
            chord.positions.map((position, index) => {
              ...
              describe('Frets', () => {
                ...
                it(`The ${index + 1} position frets array should have values lower than 16`, () =>
                  expect(Math.max(...frets)).toBeLessThan(16));
                it(`The ${index + 1} position frets array should have at most ${banjoDFsharpTuning.main.fretsOnChord} frets of distance`, () =>
                  expect(
                    effectiveFrets.length === 0
                      ? 0
                      : Math.max(...effectiveFrets) - Math.min(...effectiveFrets)
                  ).toBeLessThanOrEqual(banjoDFsharpTuning.main.fretsOnChord));
              });
