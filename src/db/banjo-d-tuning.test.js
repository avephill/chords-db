...
import banjoDTuning from './banjo-d-tuning';
import { strChord2array, processString } from '../tools';

describe('Banjo (D tuning) Chords', () => {
  describe('Strings', () => {
    it('Should have 5 strings', () =>
      expect(banjoDTuning.main.strings).toEqual(5));
    it('Should have 5 frets on chord', () =>
      expect(banjoDTuning.main.fretsOnChord).toEqual(5));
  });

  describe('Types', () => {
    banjoDTuning.suffixes.map((suffix) =>
  ...
  describe('G major', () => {
    it('Should have positions with 5-string frets', () => {
      const Gmajor = banjoDTuning.chords.G.find(
  ...
  Object.keys(banjoDTuning.chords).map((key) =>
    describe(`Key ${key}`, () => {
      const chords = banjoDTuning.chords[key];
            ...
            chord.positions.map((position, index) => {
              ...
              describe('Frets', () => {
                ...
                it(`The ${index + 1} position frets array should have values lower than 16`, () =>
                  expect(Math.max(...frets)).toBeLessThan(16));
                it(`The ${index + 1} position frets array should have at most ${banjoDTuning.main.fretsOnChord} frets of distance`, () =>
                  expect(
                    effectiveFrets.length === 0
                      ? 0
                      : Math.max(...effectiveFrets) - Math.min(...effectiveFrets)
                  ).toBeLessThanOrEqual(banjoDTuning.main.fretsOnChord));
              });
