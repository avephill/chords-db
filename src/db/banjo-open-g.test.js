...
import banjoOpenG from './banjo-open-g';
...
describe('Banjo (open G) Chords', () => {
  ...
  describe('Types', () => {
    banjoOpenG.suffixes.map((suffix) =>
  ...
  Object.keys(banjoOpenG.chords).map((key) =>
    describe(`Key ${key}`, () => {
      const chords = banjoOpenG.chords[key];
            ...
            chord.positions.map((position, index) => {
              ...
              describe('Frets', () => {
                ...
                it(`The ${index + 1} position frets array should have at most ${banjoOpenG.main.fretsOnChord} frets of distance`, () =>
                  ...
                  ).toBeLessThanOrEqual(banjoOpenG.main.fretsOnChord));
              });
